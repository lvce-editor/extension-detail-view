...
export const name = 'extension-detail.feature-webviews-invalid-id'

export const test: Test = async ({ Locator, expect, Extension, ExtensionDetail }) => {
  // arrange
  const extensionUri = import.meta.resolve('../fixtures/extension-webviews-invalid-content-security-policy')
  await Extension.addWebExtension(extensionUri)
  await ExtensionDetail.open('test.webviews-invalid-content-security-policy')
  ...
  await expect(item3Value).toHaveText(`{"value":["default-src 'none'","script-src 'self'"]}`)
}
