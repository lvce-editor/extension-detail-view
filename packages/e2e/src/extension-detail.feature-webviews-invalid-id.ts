...
export const test: Test = async ({ expect, Extension, ExtensionDetail, Locator }) => {
  // arrange
  const extensionUri = import.meta.resolve('../fixtures/extension-webviews-invalid-id')
  await Extension.addWebExtension(extensionUri)
  await ExtensionDetail.open('test.webviews-invalid-id')
  ...
  await expect(item1Value).toHaveText('[object Object]')
