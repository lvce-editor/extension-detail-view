...
export const name = 'extension-detail.feature-json-validation-link-number'

export const test: Test = async ({ Locator, expect, Extension, ExtensionDetail }) => {
  // arrange
  const extensionUri = import.meta.resolve('../fixtures/extension-json-validation-schema-link-number')
  await Extension.addWebExtension(extensionUri)
  await ExtensionDetail.open('test.json-validation-schema-link-number')
  ...
  await expect(cell2).toHaveText('123')
