...
export const test: Test = async ({ Locator, expect, Extension, ExtensionDetail }) => {
  ...
  const cell3 = programmingLanguagesTable.locator('tbody td').nth(2)
  await expect(cell3).toHaveText('.css')
  const cell4 = programmingLanguagesTable.locator('tbody td').nth(3)
  await expect(cell4).toHaveText('yes')
  const cell5 = programmingLanguagesTable.locator('tbody td').nth(4)
