...
test('loadContent - successful load', async () => {
  ...
  expect(result.cacheName).toBe('test-app/extension-readme-markdown-cache')
...
test('loadContent - loads the selected feature details when restoring the features tab', async () => {
  ...
  expect(getDetails).toHaveBeenCalledWith(mockExtension, '/test/path', 'https:', 'test-app/extension-readme-markdown-cache')
